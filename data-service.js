...
// Hämta aktiva onlinetävlingar (Publik funktion)
export async function getActiveOnlineCompetitions() {
    // Vi hämtar alla och filtrerar i frontend för enkelhetens skull, eller kör en query
    const q = query(collection(db, 'online_competitions'), orderBy('endDate', 'desc'));
    const snapshot = await getDocs(q);
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
}

// Hämta mina externa skyttar
export async function getMyExternalShooters(userId) {
    if (!userId) return [];
    const q = query(collection(db, 'external_shooters'), where('ownerUserId', '==', userId));
    const snapshot = await getDocs(q);
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
}

// Lägg till extern skytt
export async function addExternalShooter(userId, data) {
    if (!userId) return;
    try {
        await addDoc(collection(db, 'external_shooters'), {
            ...data,
            ownerUserId: userId,
            createdAt: serverTimestamp()
        });
        showModal('confirmationModal', "Skytt tillagd!");
    } catch (error) {
        console.error("Fel vid tillägg av skytt:", error);
        showModal('errorModal', "Kunde inte spara skytten.");
    }
}

// Ta bort extern skytt
export async function deleteExternalShooter(shooterId) {
    // OBS: I framtiden bör vi kolla så man inte tar bort en skytt som har resultat i en pågående tävling
    try {
        await deleteDoc(doc(db, 'external_shooters', shooterId));
        showModal('confirmationModal', "Skytt borttagen.");
    } catch (error) {
        console.error("Fel vid borttagning:", error);
        showModal('errorModal', "Kunde inte ta bort skytten.");
    }
}
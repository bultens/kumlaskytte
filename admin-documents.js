...
import { isAdminLoggedIn } from "./ui-handler.js"; // IMPORTERA DENNA
...
export async function initFileManager() {
    ...
    // Försök ladda roten, men loadFolder kommer avbryta om vi inte är admin
...
async function loadFolder(folderId) {
    // VIKTIGT: Om man inte är admin, gör ingenting för att undvika "Permission Denied"
    ...
    files.forEach(file => {
        ...
        div.innerHTML = `
            ...
            <div class="flex items-center gap-4 flex-shrink-0">
                ...
                <div class="relative">
                    <button class="p-2 text-gray-500 hover:text-gray-800 font-bold rounded-full hover:bg-gray-200" data-action="toggle-menu" data-id="${file.id}">⋮</button>
                    <div id="file-menu-${file.id}" class="hidden absolute right-0 mt-2 w-48 bg-white border rounded shadow-xl z-50">
                        <button class="w-full text-left px-4 py-2 hover:bg-gray-100 text-sm" data-action="move-file" data-id="${file.id}">↪ Flytta...</button>
                        <button class="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 text-sm font-bold border-t" data-action="delete-file" data-id="${file.id}" data-storage-path="${file.storagePath}">🗑️ Ta bort fil</button>
                    </div>
                </div>
            </div>
        `;
...
function updateBreadcrumbs() {
    ...
    el.innerHTML = breadcrumbPath.map((crumb, index) => {
        ...
        return `<span class="text-blue-600 cursor-pointer hover:underline" data-action="open-folder" data-id="${crumb.id !== null ? crumb.id : 'null'}" data-name="${crumb.name}">${crumb.name}</span> <span class="text-gray-400 mx-2">/</span>`;
    }).join('');
}
...
async function moveFile(docId) {
    ...
    folders.forEach((f, i) => { folderListText += `${i + 1}: ${f.name}\n`; });
